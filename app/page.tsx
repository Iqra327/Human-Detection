...
useEffect(() => {
  //initialize the 
  if (webcamRef && webcamRef.current && !audioStream) {
    const stream = (webcamRef.current.video as any).captureStream(30); // Set the frame rate to 30 fps
    if (stream) {
      navigator.mediaDevices.getUserMedia({ audio: true }).then((newAudioStream) => {
        const newTracks = newAudioStream.getTracks();
        const newAudioTrack = newTracks[0];
        newAudioTrack.enabled = true;

        const updatedStream = new MediaStream();
        updatedStream.addTrack(stream.getVideoTracks()[0] as MediaStreamTrack);
        updatedStream.addTrack(newAudioTrack);

        setAudioStream(updatedStream);
      });
    }
  }
}, [webcamRef, audioStream]);
